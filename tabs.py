...
def get_forecast_tab(dropdown_options):
    return [
              ...
              html.Div(
                [
                  html.Div('The points show the daily cumulated cases, while the line shows the logistic fit with uncertainty (shaded area).', style={'margin-left': 10}),
                  html.Div('In the right inset the parameters obtained from the logistic fit are shown: note that these are only parameters, not reliable forecasts!', style={'margin-left': 10}),
                  html.Div('The value of R2 is also shown: the closer to 1 the better the fit.', style={'margin-left': 10}),
