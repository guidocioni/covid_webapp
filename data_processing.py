...
'''here we do the preprocessing of the data that is going to be read in the appliation to avoid waiting times
on the frontend'''
...
def read_jrc():
    ...
    df['daily_cases'] = df.groupby("Region")['CumulativePositive'].transform(lambda x: x.diff())
    df['daily_deaths'] = df.groupby("Region")['CumulativeDeceased'].transform(lambda x: x.diff())
    df['daily_recovered'] = df.groupby("Region")['CumulativeRecovered'].transform(lambda x: x.diff())
    df['daily_cases_smoothed'] = df.groupby("Region")['CumulativePositive'].transform(lambda x: x.diff().rolling(7).mean())
    df['daily_deaths_smoothed'] = df.groupby("Region")['CumulativeDeceased'].transform(lambda x: x.diff().rolling(7).mean())
    df['daily_recovered_smoothed'] = df.groupby("Region")['CumulativeRecovered'].transform(lambda x: x.diff().rolling(7).mean())
...
df_weekly_ecdc = read_weekly_ecdc().to_pickle(TMP_FOLDER+'df_weekly_ecdc.pickle')
df_hospitalization = read_hospitalization().to_pickle(TMP_FOLDER+'df_hospitalization.pickle')
